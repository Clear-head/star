import os
import pandas as pd
from Setting import add_fdata_list
import re
...
def to_Fdata(folder_path):
    ...
    for file in os.listdir(folder_path):
        if file.endswith('.xls') and file not in excel_list:
            df_list.append(pd.read_excel(os.path.join(folder_path, file), usecols=col, engine='xlrd'))
    ...
    while not df.empty:
        ...
        output_file = os.path.join(Fdata_path, f"{str(start_date)[:7]}.csv")
        monthly_df.to_csv(output_file, index=False, encoding="utf-8-sig")
        add_fdata_list(f"{str(start_date)[:7]}.csv", 'True', os.path.join(folder_path, 'set'))
